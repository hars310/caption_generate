...
function getClient() {
  return new TranscribeClient({
    ...
    credentials: {
      accessKeyId: process.env.HARSH_AWS_ACCESS_KEY,
      secretAccessKey: process.env.HARSH_AWS_SECRET_ACCESS_KEY,
    },
  });
}
function createTranscriptionCommand(filename) {
  return new StartTranscriptionJobCommand({
    ...
    OutputBucketName: process.env.HARSH_BUCKET_NAME,
    OutputKey: filename + '.transcription',
    IdentifyLanguage: true,
    Media: {
      MediaFileUri: 's3://' + process.env.HARSH_BUCKET_NAME + '/'+filename,
    },
  });
}
...
async function getTranscriptionFile(filename) {
  ...
  const getObjectCommand = new GetObjectCommand({
    Bucket: process.env.HARSH_BUCKET_NAME,
